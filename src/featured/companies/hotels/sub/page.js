...
export default function HotelSubPage () {
    ...
    if (!details) return <div>No se encontraron detalles para este comercio.</div>;
    ...
    const schemaData = {
        ...
        "url": `https://andaleya.pe/hotels/${details?.sub}`,
        "brand": {
            "@type": "Brand",
            "name": "Ándale Ya!",
        },
        ...
        <>
            ...
            <main className="__main_company">
                ...
                <section className={`__section_company __section_company_lst`}>
                    <ul className="__lst">
                        {details?.bedrooms.length > 0 ? (
                            details?.bedrooms.map((pdt) => (
                                <li key={pdt.id}>{pdt.name}</li>
                            ))
                        ): (
                            <div><p>No hay cuartos disponibles.</p></div>
                        )}
                    </ul>
                </section>

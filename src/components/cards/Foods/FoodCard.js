...
export default function FoodCard ({ buss, food, filter }) {
    ...
    return (
        ...
        <div className={`__card __card_food ${filter !== 'all' && filter !== food?.category ? '__card_food--hidden' : ''}`}  style={{backgroundImage: `url(${image})`}} onClick={() => handleChangeModal('food', buss?.id, food?.id)}>
